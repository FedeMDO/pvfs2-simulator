__includes["aplicacion.nls" "mpi_io.nls" "adio.nls" "ad_pvfs.nls" "system_interface.nls" "job_cliente.nls" "flow_cliente.nls" "bmi_cliente.nls" "comunicacion.nls"
  "main_loop.nls" "job_servidor.nls" "flow_servidor.nls" "bmi_servidor.nls" "trove.nls" "mdt_main_loop.nls" "regresion.nls" "regresion_megas.nls"]
...
datas-own[
  ...
  KiB ;obtiene KiB por función
  unidad ; para sys_pvfs se obtiene de unidad. Para el resto es KiB
]
...
to setup
  ...
  ask mdt_main_loops[
    if (capainf = nobody)[
      ...
      ;show x
      ;configurar_agentes_servidor -26 7 11.3 9.6 7.9 6.2 4 true
      configurar_agentes_servidor_mdt -5 -3.5 -6.5 -9.5 -12.5 -15.5 7 false
      ;configurar_agentes_servidor -26 7 11.3 9.6 7.9 6.2 4 true
    ]
  ]
...
;    set pcolor lime + 4 ]
  ...
  ;;bus
   ...
   ;;metadataservers
   ask patches with [(pxcor >= -5 and pycor <= 0) and (pxcor <= 36 and pycor >= -18)][
    set pcolor gray + 3]
...
to configurar_agentes_servidor_mdt [x y y1 y2 y3 y4 tamaño oculto?]
  ...
  if(count mdt_main_loops-here > 5)[
    set hidden? false
  ]

  ifelse(oculto? = false)[ setxy (x + (count mdt_main_loops-here * 7)) y] [setxy (x + (count mdt_main_loops-here * 10)) (y)] ;-8 -2
  ;setxy (x + (count main_loops-here )) (y) ;-8 -2
  set size tamaño
  if(oculto? = false)[
    agregar_etiqueta -2 y
  ...
  ask capainf[
    ...
    setxy [xcor] of capasup y1
    set size tamaño
    if(oculto? = false)[agregar_etiqueta -2 y1]
    set hidden? oculto?
    ...
    ask capainf_f[
      set capasup id_job
      set size tamaño
      if(oculto? = false)[agregar_etiqueta -2 y2]
      set hidden? oculto?
      set capainf_b [capainf_b] of capasup
      set capainf_t [capainf_t] of capasup
      setxy [xcor] of capasup y2
    ...
    ask capainf_b[
      set capasup_j id_job
      set size tamaño
      if(oculto? = false)[agregar_etiqueta -2 y3]
      set hidden? oculto?
      set capasup_f [capainf_f] of capasup_j
      setxy [xcor] of capasup_j y3
    ]
    ask capainf_t[
      set capasup_j id_job
      set size tamaño
      if(oculto? = false)[agregar_etiqueta -2 y4]
      set hidden? oculto?
      set capasup_f [capainf_f] of capasup_j
      setxy [xcor] of capasup_j y4
    ]
